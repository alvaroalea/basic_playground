...
70 FOR b = 0 TO 32 * 20 :  POKE @mapa + b , 0 :  NEXT b
80 RESTORE 40
90 READ a
100 IF a = 0 THEN  GOTO 210
110 IF a = 1 THEN  LET p$ = "\H" :  INK 3
120 IF a = 2 THEN  LET p$ = "_" :  INK 7
130 IF a = 10 THEN  LET p$ = "\G" :  INK 5
140 READ n :  READ x :  READ y
150 FOR c = 1 TO n :  PRINT  AT y , x ; p$
160 POKE @mapa +  ( 32 * y )  + x , a
...
260 GOSUB 930 : REM imprimir marcador
    frame = PEEK 23672 + 256*PEEK 23673 + 65536*PEEK 23674
    pframe = frame

269 REM 270 - INICIO BUCLE JUEGO
270 frame = PEEK 23672 + 256*PEEK 23673 + 65536*PEEK 23674
    if frame < (pframe + 10) goto 270
    pframe = frame
    LET key$ =  INKEY$ 
280 IF key$ = "o" AND  PEEK  ( @mapa +  ( 32 *  ( cy + 1 )  )  + cx - 1 )  <> 0 THEN  LET cx1 = cx - 1 :  GOTO 330
290 IF key$ = "p" AND  PEEK  ( @mapa +  ( 32 *  ( cy + 1 )  )  + cx + 1 )  <> 0 THEN  LET cx1 = cx + 1 :  GOTO 330
300 IF key$ = "a" AND  PEEK  ( @mapa +  ( 32 *  ( cy + 1 )  )  + cx )  = 10 THEN  LET cy1 = cy + 1 :  GOTO 330
310 IF key$ = "q" AND  PEEK  ( @mapa +  ( 32 *  ( cy )  )  + cx )  = 10 THEN  LET cy1 = cy - 1 :  GOTO 330
...
380 LET r =  RND 
390 IF r < 0.25 AND  PEEK  ( @mapa +  ( 32 *  ( py + 1 )  )  + px - 1 )  <> 0 THEN  LET px1 = px - 1 :  GOTO 440
400 IF r < 0.5 AND  PEEK  ( @mapa +  ( 32 *  ( py + 1 )  )  + px + 1 )  <> 0 THEN  LET px1 = px + 1 :  GOTO 440
410 IF r < 0.75 AND  PEEK  ( @mapa +  ( 32 *  ( py + 1 )  )  + px )  = 10 THEN  LET py1 = py + 1 :  GOTO 440
420 IF  PEEK  ( @mapa +  ( 32 *  ( py )  )  + px )  = 10 THEN  LET py1 = py - 1 :  GOTO 440
...
460 IF my > cy AND  PEEK  ( @mapa +  ( 32 *  ( my )  )  + mx )  = 10 THEN  LET my1 = my - 1 :  GOTO 510
470 IF my < cy AND  PEEK  ( @mapa +  ( 32 *  ( my + 1 )  )  + mx )  = 10 THEN  LET my1 = my + 1 :  GOTO 510
480 IF cx < mx AND  PEEK  ( @mapa +  ( 32 *  ( my + 1 )  )  + mx - 1 )  <> 0 THEN  LET mx1 = mx - 1 :  GOTO 510
490 IF  PEEK  ( @mapa +  ( 32 *  ( my + 1 )  )  + mx + 1 )  <> 0 THEN  LET mx1 = mx + 1 :  GOTO 510
500 LET mx1 = mx1 - 1
...
530 IF  PEEK  ( @mapa +  ( 32 *  ( my - 1 )  )  + mx )  = 10 THEN  PRINT  INK 5 ;  AT my - 1 , mx ; "\G" :  GOTO 550
540 PRINT  AT my - 1 , mx ; " "
550 IF  PEEK  ( @mapa +  ( 32 *  ( my )  )  + mx )  = 10 THEN  PRINT  INK 5 ;  AT my , mx ; "\G" :  GOTO 570
560 PRINT  AT my , mx ; " "
570 LET my = my1 :  LET mx = mx1
580 PRINT  INK 6 ;  AT my - 1 , mx ; "\K" ;  AT my , mx ; "\L"
590 RETURN

599 REM 600 - RUTINA IMPRIMIR el bicho P 
600 IF  PEEK  ( @mapa +  ( 32 *  ( py - 1 )  )  + px )  = 10 THEN  PRINT  INK 5 ;  AT py - 1 , px ; "\G" :  GOTO 620
610 PRINT  AT py - 1 , px ; " "
620 IF  PEEK  ( @mapa +  ( 32 *  ( py )  )  + px )  = 10 THEN  PRINT  INK 5 ;  AT py , px ; "\G" :  GOTO 640
630 PRINT  AT py , px ; " "
640 LET py = py1 :  LET px = px1
650 PRINT  INK 4 ;  AT py - 1 , px ; "\M" ;  AT py , px ; "\N"
660 RETURN

669 REM 670 - RUTINA IMPRIMIR VIDAS 
670 LET a = 20 -  ( 3 * vidas ) 
680 FOR c = 5 TO 20 STEP 3
690 PRINT  INK 7 ;  AT c , 0 ; " " ;  AT c + 1 , 0 ; " "
700 IF c > a THEN  PRINT  INK 7 ;  AT c , 0 ; "\I" ;  AT c + 1 , 0 ; "\J"
710 NEXT c
720 RETURN
...
770 IF  PEEK  ( @mapa +  ( 32 * cy )  + cx )  = 10 THEN  PRINT  INK 5 ;  AT cy , cx ; "\G" :  GOTO 790
780 PRINT  AT cy , cx ; " "
790 IF  PEEK  ( @mapa +  ( 32 *  ( cy - 1 )  )  + cx )  = 10 THEN  PRINT  INK 5 ;  AT cy - 1 , cx ; "\G" :  GOTO 810
800 PRINT  AT cy - 1 , cx ; " "
810 PRINT  INK 7 ;  AT cy1 - 1 , cx1 ; "\I" ;  AT cy1 , cx1 ; "\J"
820 LET cx = cx1 :  LET cy = cy1
830 IF  PEEK  ( @mapa +  ( 32 *  ( 1 + cy )  )  + cx )  = 2 THEN  GOSUB 1260
840 RETURN 
...
870 PRINT  AT 3 , 26 ; "\I" ;  AT 4 , 26 ; "\J"
880 PRINT  AT 2 , 5 ;  INK 2 ; "\A\B\C" ;  AT 4 , 5 ; "\A\B\C" ;  AT 2 , 21 ; "\A\B\C" ;  AT 4 , 21 ; "\A\B\C" ;  INK 4 ;  AT 3 , 5 ; "\D\E\F" ;  AT 3 , 21 ; "\D\E\F" ;  INK 3 ;  AT 5 , 0 ; "\H\H\H\H\H\H\H\H\H\H\H\H\H\H\H\H\H\H\H\H\H\H\H\H\H\H\H\H\H\H\H\H"
...
9980 REM CLEAR 49999 :  
     RANDOMIZE  :  LET nc = 8 :  LET hipunt = 0 :  GOSUB 9610 :  GOTO 850 : REM UDG y luego MENU PRINCIPAL.
